								...
								__global ulong *restrict vhash)
{
	...
	// printf("lookups: %d, finished nucs_idx\n", lookups);
...
//	{
		...
		for( int j = 0; j < num_nucs[lc.mat]; j++ )
		{
			...
			// printf("num_nucs[%d]: %d, p_nuc: %d, lc.idx: %ld\n", lc.mat, num_nucs[lc.mat], p_nuc, lc.idx);
			// calculate_micro_xs
			NuclideGridPoint low, high;
			long energy_at_nuc = (long) energy_grid_xs[lc.idx * n_isotopes + p_nuc]; // energy_grid_xs_bin[p_nuc];
			// printf("finised enery_at_nuc: %ld\n", energy_at_nuc);
...
//	}
	...
	for(int k = 0; k < 5; k++)
		vhash[gid % 1024] += macro_xs_vector[k];
}
